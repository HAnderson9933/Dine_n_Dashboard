...
module.exports = {
    ...
    findAllRestaurantsByUser: (req, res) =>{
        console.log("All the User's Restaurants!");
        Restaurant.find({user_id: req.params.id})
        .then((allUsersRestaurants) =>{
            res.json(allUsersRestaurants);
        })
        .catch((err) =>{
            console.log("Get all user's restaurants failed");
            res.status(400).json(err);
        })
    },
